...
$sql = mysqli_query($link, "SELECT * FROM `catalog`");
...
<body>
    ...
    <div class="container-fluid d-flex w-100 h-100 mx-auto flex-column">
        <h1 class="h-100 sec1 text-center px-1">Товары</h1>
        <div class="sec1 h-100 d-flex actions">
            <?php
                $sql = mysqli_query($link, "SELECT* FROM `catalog` WHERE `id_k`='".$_GET['id_c']."'");
                while ($result = mysqli_fetch_array($sql)) {
                    echo "<div class='card bonaf' style='width: 28rem;'> <img src=img/{$result['img']} class='card-img-top' alt='#'> 
                    <div class='card-body text-center'> <h5 class='card-title'>{$result['opis']}</h5> 
                   
                    <h5 class='card-title text-primary'>{$result['sum']} РУБ</h5>  
                    </div></div>";
                }
            ?>
        </div>
